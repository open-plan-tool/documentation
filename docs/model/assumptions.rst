...
The open-plan-tool uses the programming framework :code:`oemof-solph` at its core and builds an energy system model based upon its nomenclature.
...
open-plan-tool is based on :code:`oemof-solph`, which in turn uses :code:`Pyomo` to create a linear problem.
The economic dispatch problem in the open-plan-tool has the objective of minimizing the production cost by allocating the total demand among the generating units at each time step.
...
open-plan-tool considers a linear depreciation over :math:`T` and accounts for the time value of money through the use of the following equation:
...
For each bus in the system, the open-plan-tool automatically includes a sink component for excess energy related to the bus, which is denoted :math:`E_{ex}` in the equations.
...
The optimization of the open-plan-tool is mainly based on costs.
